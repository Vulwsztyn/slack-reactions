...
async function main() {
    ...
    const messages = (await axios.post('https://slack.com/api/conversations.history', {
        channel: channelId
    ...
    console.log(
        messages.map(m => m.text.slice(0, 10))
    )
    console.log({text})
    const regex = new RegExp(`${text}`, 'i')
    const message = messages.find(m => m.text.match(regex))
    console.log(
        messages.map(m => m.text.match(regex))
    )
    console.log({message})
    const used = {}
    for (const reaction of reactions) {
        const colour = used[reaction] ? 'yellow' : 'white'
        const name = `alphabet-${colour}-${reaction}`
